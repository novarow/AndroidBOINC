...
bool run_slow = true; // sleep thirty  seconds after every letter
...
int main(int argc, char **argv) {
	...
	char msg[1024];
	boinc_getcwd(cwd);
        snprintf(msg,sizeof(msg),"Hello World, this is UC! current working directory is: %s",cwd);
	LOGD(msg);
#endif
    int i;
...
LOGD("APP_GRAPHICS is defined!");
    ...
    for (i=0; ; i++) {
        ...
        nchars++;
        if (run_slow) {
LOGD("run slow... sleep for 30 seconds");
            boinc_sleep(30.);
        }
