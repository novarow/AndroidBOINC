...
//LOGD("app_control: ACTIVE_TASK_SET::poll");
    bool action;
    unsigned int i;
...
LOGD("app_control: ACTIVE_TASK::request_exit");
...
LOGD("app_control: ACTIVE_TASK::request_abort");
...
LOGD("app_control: static kill_app_process");
...
LOGD("app_control: ACTIVE_TASK::kill_task");
...
LOGD("app_control: ACTIVE_TASK::has_task_exited");
...
LOGD("app_control: static limbo_message");
#ifdef _WIN32
...
LOGD("app_control: ACTIVE_TASK::handle_premature_exit");
    switch (task_state()) {
...
LOGD("app_control: ACTIVE_TASK::handle_exited_app");
#endif
...
LOGD("app_control: ACTIVE_TASK::finish_file_present");
    char path[256];
...
LOGD("app_control: ACTIVE_TASK::temporary_exit_file_present");
    char path[256];
...
//LOGD("app_control: ACTIVE_TASK_SET::send_trickle_downs");
    unsigned int i;
    ACTIVE_TASK* atp;
    bool sent;
    for (i=0; i<active_tasks.size(); i++) {
...
//LOGD("app_control: ACTIVE_TASK_SET::send_hearbeats");
    unsigned int i;
    ACTIVE_TASK* atp;
    char buf[1024];
...
LOGD("app_control: ACTIVE_TASK::check_max_disk_exceeded");
    double disk_usage;
    int retval;
...
LOGD("app_control: ACTIVE_TASK::abort_task");
...
LOGD("app_control: ACTIVE_TASK::request_reread_prefs");
    int retval;
    APP_INIT_DATA aid;
...
LOGD("app_control: ACTIVE_TASK::request_reread_app_info");
    APP_INIT_DATA aid;
    init_app_init_data(aid);
...
LOGD("app_control: ACTIVE_TASK_SET::request_reread_prefs");
    unsigned int i;
    ACTIVE_TASK* atp;
...
LOGD("app_control: ACTIVE_TASK_SET::request_reread_app_info");
...
LOGD("app_control: ACTIVE_TASK_SET::exit_tasks");
    if (log_flags.task_debug) {
...
LOGD("app_control: ACTIVE_TASK_SET::wait_for_exit");
    bool all_exited;
    unsigned int i,n;
    ACTIVE_TASK *atp;
...
LOGD("app_control: ACTIVE_TASK_SET::abort_project");
...
LOGD("app_control: ACTIVE_TASK_SET::suspend_all");
    for (unsigned int i=0; i<active_tasks.size(); i++) {
        ACTIVE_TASK* atp = active_tasks[i];
        if (atp->task_state() != PROCESS_EXECUTING) continue;
        switch (reason) {
...
LOGD("app_control: ACTIVE_TASK_SET::unsuspend_all");
    unsigned int i;
    ACTIVE_TASK* atp;
    for (i=0; i<active_tasks.size(); i++) {
        atp = active_tasks[i];
...
LOGD("app_control: ACTIVE_TASK_SET::is_task_executing");
    unsigned int i;
    ACTIVE_TASK* atp;
    for (i=0; i<active_tasks.size(); i++) {
        atp = active_tasks[i];
...
LOGD("app_control: ACTIVE_TASK::kill_tasks");
    unsigned int i;
    ACTIVE_TASK *atp;
    for (i=0; i<active_tasks.size(); i++) {
        atp = active_tasks[i];
...
LOGD("app_control: ACTIVE_TASK::suspend");
...
LOGD("app_control: ACTIVE_TASK::unsuspend");
    if (!app_client_shm.shm) return 0;
...
LOGD("app_control: ACTIVE_TASK::send_network_available");
...
//LOGD("app_control: ACTIVE_TASK::get_app_status_msg");
...
//LOGD("app_control: ACTIVE_TASK::get_trickle_up_msg");
    char msg_buf[MSG_CHANNEL_SIZE];
    bool found = false;
    int retval;
...
LOGD("app_control: ACTIVE_TASK::write_task_state_file");
    char path[1024];
...
LOGD("app_control: ACTIVE_TASK::read_task_state_file");
