...
LOGD("GUI_RPC_CONN_SET::get_password()");
    FILE* f;
    int retval;

    strcpy(password, "");
    if (boinc_file_exists(GUI_RPC_PASSWD_FILE)) {
LOGD("password file exists, get pwd from there");
...
LOGD("no password file exists");
        // if no password file, make a random password
        //
        retval = make_random_string(password); //at hostinfo_network.cpp
#ifdef ANDROID
	char msg[1024];
	snprintf(msg,sizeof(msg),"random string: %s with retval: %d",password,retval);
	LOGD(msg);
#endif
LOGD("after password");
        if (retval) {
...
LOGD("GUI_RPC_CONN_SET::get_allowed_hosts");
    int retval;
...
#ifdef ANDROID
	char msg[1024];
	snprintf(msg,sizeof(msg),"socket binding returned: %d",retval);
	LOGD(msg);
#endif
    if (retval) {
#ifndef _WIN32
