...
#ifdef ANDROID
	LOGD("FILE_INFO::verfiy_file");
	if(is_image_file(name) && size>0) {
		char msg[1024];
		snprintf(msg,sizeof(msg),"gstate.global_prefs.dont_verify_images is: %d",gstate.global_prefs.dont_verify_images);
		LOGD(msg);
		return 0;
	}
#else
