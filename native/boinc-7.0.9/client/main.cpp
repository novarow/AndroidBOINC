...
#include "android_log.h" //setup logcat for debugging android application via DDMS
...
LOGD("log_message_startup");
	...
	__android_log_print(ANDROID_LOG_DEBUG, "main::log_message_startup", evt_msg);
#else
...
#ifdef ANDROID
LOGD("log_message_error with code");
#endif
...
static void init_core_client(int argc, char** argv) {
    setbuf(stdout, 0);
    setbuf(stderr, 0);
    config.defaults(); //location: lib/prefs.cpp
...
LOGD("about to read_config_file");
    read_config_file(true);//location: client/log_flags.cpp
...
LOGD("initialize complete");

    retval = gstate.init(); //gstate.init at client/client_state.cpp
LOGD("gstate.init complete");
    if (retval) {
...
int main(int argc, char** argv) {
    ...
    char msg[1024];
    snprintf(msg,sizeof(msg),"Android current working directory at: %s", ccwd);
    LOGD(msg);
#endif
...
#endif  // SANDBOX
LOGD("starting boinc_main_loop()");
    retval = boinc_main_loop();

char cretval[1024];
snprintf(cretval,sizeof(cretval),"boinc about to exit with status: %d",retval);
LOGD(cretval);
#endif
    return retval;
}
