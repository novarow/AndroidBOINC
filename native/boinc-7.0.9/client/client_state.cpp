...
LOGD("CLIENT_STATE::show_host_info");
...
LOGD("CLIENT_STATE::init()");
    ...
    relative_to_absolute("", buf); //located in filesystem.cpp
...
LOGD("setting up socket and rpc");
        ...
        for (i=0; i<30; i++) {
            bool last_time = (i==29);
            retval = gui_rpcs.init(last_time); //at gui_rpc_server.cpp
            if (!retval) break;
            boinc_sleep(1.0);
        }
        if (retval) return retval;
