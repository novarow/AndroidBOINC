...
#ifdef ANDROID
//	char msg[1024];
//	snprintf(msg,sizeof(msg),"filesys.cpp dir_open path: %s",p);
//	LOGD(msg);
#endif
    DIRREF dirp;
#ifdef _WIN32
...
#ifdef ANDROID
//	char msg[1024];
//	snprintf(msg,sizeof(msg),"DirScanner:: path: %s",path.c_str());
//	LOGD(msg);
#endif
}
...
#else
//char cdirp[1024];
//snprintf(cdirp,sizeof(cdirp),"dirp: %s",dirp);
//LOGD(cdirp);
...
#ifdef ANDROID
//	        char msg[1024];
//       		 snprintf(msg,sizeof(msg),"DirScanner::scan: %s",s.c_str());
//       		 LOGD(msg);
#endif

            return true;
        } else {
//LOGD("DirScanner::scan ELSE");
return false;
        }
    }
#endif
...
#ifdef ANDROID
//	char msg[1024];
//	snprintf(msg,sizeof(msg),"boinc_fopen path: %s, mode: %s",path,mode);
//	LOGD(msg);
#endif
...
#ifdef ANDROID
	char msg[1024];
	snprintf(msg,sizeof(msg),"filesys.cpp boinc_mkdir: %s",path);
	LOGD(msg);
#endif
...
#ifdef ANDROID
	char msg[1024];
	sprintf(msg,"FILE_LOCK::lock fd: %d for file: %s",fd,filename);
	LOGD(msg);
	char cErrno[10];
	sprintf(cErrno, "errno: %d",errno);
	LOGD(cErrno);
    }
#endif
    if (fd<0) {
...
#ifdef ANDROID
char msg[1024];
snprintf(msg,sizeof(msg),"boinc_getcwd in filesys.cpp: %s", path);
LOGD(msg);
#endif
}
...
#ifdef ANDROID
//char debug[100];
//snprintf(debug,sizeof(debug),"relative_to_absolute setup name: %s for path: %s",relname,path);
//LOGD(debug);
#endif
}
...
#ifdef ANDROID
//	char msg[1024];
//	snprintf(msg,sizeof(msg),"filesys.cpp get_file_dir filename: %s, dir: %s",filename,dir);
//	LOGD(msg);
#endif
